          ...
          {activeTab === 'manual' && (
            <form id="orderForm" onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="p-4 bg-white rounded-xl border border-gray-200 shadow-sm">
                    ...
                    {/* Valore commessa */}
                    <div className="mb-4">
                        <label className="block font-bold text-gray-500 uppercase mb-1" style={{ fontSize: `${scaleFontSize(10)}px` }}>{t('order_value')}</label>
                        <div className="relative">
                            <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500 font-bold text-sm">€</span>
                            <input
                                type="number" min="0" step="0.01"
                                value={formData.orderValue ?? ''}
                                onChange={e => setFormData({...formData, orderValue: e.target.value ? Number(e.target.value) : undefined})}
                                className="w-full pl-7 pr-3 py-2.5 border border-gray-200 rounded-lg font-bold text-gray-700 text-right"
                                placeholder="0.00"
                            />
                        </div>
                    </div>
